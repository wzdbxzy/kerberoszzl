...
from PyQt5.QtWidgets import QApplication, QMainWindow, QMessageBox, QWidget, QLabel, QLineEdit, QPushButton, \
    QTableWidget, QTableWidgetItem, QVBoxLayout, QHBoxLayout, QTextEdit
...
SERVER_HOST = '192.168.47.149'  # 服务器的主机名
...
class LoginWindow(QWidget):
    ...
    def authenticate_with_as(self, username, password):
        ...
        # 发送用户名和密码给AS
        as_socket.sendall(f'{username}:{password}:{expire_time}'.encode())
        ...
        if 'tgt' in response_data:
            ...
            now = int(time.time())
            expire_time = now + lifetime
            new_response_data = pickle.dumps({'tgt': entgt, 'encrypted_session_key': session_key, 'time': expire_time})
    ...
    def request_server_access1(self, encrypted_server_ticket, username, password):
        ...
        try:
            ...
            print("签名验证成功！消息未被篡改。")
            print('1')
            subprocess.run(['python', 'book_manager.py', username, password])
...
def load_file(text_widget):
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        text_widget.delete(3.0, tk.END)  # 清空Text小部件内容
        text_widget.insert(tk.END, file_content)  # 在Text小部件中插入文件内容


# 创建Tkinter窗口
window = tk.Tk()
# 调整窗口的大小
window.geometry("800x600")  # 设置宽度为600像素，高度为400像素
# 创建Text小部件
text_widget = tk.Text(window)
window.title("Client Output")  # 设置窗口标题
text_widget.pack()
# 调整字体大小
font_style = font.Font(size=30)  # 设置字号为30
text_widget.configure(font=font_style)
# 加载文件并显示内容
load_file(text_widget)
