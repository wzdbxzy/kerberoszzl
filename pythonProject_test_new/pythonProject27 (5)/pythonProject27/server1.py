...
# 定义全局变量
# 定义全局变量
SEREVER_HOST = '192.168.47.149'  # TGS的主机名
SEREVER_PORT = 5002  # TGS的端口号
...
# 创建一个 Tkinter 窗口
window = tk.Tk()
window.title("V Server Output")  # 设置窗口标题
...
def serever_server():
    ...
    print(f'Vserever 服务器正在监听 {SEREVER_HOST}:{SEREVER_PORT}')
    ...
    while True:
        ...
        if 'tgt' in response_data:
        ...
        elif 'shan' in response_data:
            # 提取TGT和会话密钥
            encrypted_ticket = response_data['shan']

            session_key = response_data['encrypted_session_key']
            book_id = response_data['title']
            key = response_data['key']
            print(session_key)
        ...
        elif 'cha' in response_data:
            # 提取TGT和会话密钥
            encrypted_ticket = response_data['cha']
            ...
            try:
                ...
                if validate_tgt2(message):
                    ...
                    # 发送消息和签名
                    # 在实际应用中，需要将消息和签名一起发送给验证方（例如，通过网络传输）
                    response = pickle.dumps({'tgt': message, 'encrypted_session_key': signature1, 'books': books})
                    print(response)
                    client_socket.sendall(response)
...
def validate_tgt1(tgt):
    return tgt.endswith(b'shan')


def validate_tgt2(tgt):
    return tgt.endswith(b'cha')
