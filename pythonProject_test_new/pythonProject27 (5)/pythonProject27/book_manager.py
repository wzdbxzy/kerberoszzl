...
class BookManager(QMainWindow):
    ...
    def query_book(self):
        ...
        # 打印服务器的响应
        print(f'Server Response: {server_response}')
        server_response1 = self.request_server_access3(server_response)
        ...
        # 在表格中显示查询结果
        self.bookTable.setRowCount(len(server_response1))
        for i in range(len(server_response1)):
            row = server_response1[i]
    ...
    def request_server_access3(self, encrypted_server_ticket):
        ...
        message1 = self.encrypt_des(message2, session_key)
        response = pickle.dumps(
            {'cha': message1, 'encrypted_session_key': signature1, 'key': session_key})
        server_socket.sendall(response)
        response_data = b''
        while True:
            data = server_socket.recv(1024)
